  ...
  Image: screenshot → https://i.imgur.com/fhGqC9U.png                         
                                                                              
  With nerd fonts:                                                            
  Image: with-nerd-fonts → https://i.imgur.com/aXI5BTG.png                    
                                                                              
  Video presentation by TJ:                                                   
                                                                              
  Image: Video presentation by TJ →                                           
  https://i.ytimg.com/vi/ALGBuFLzDSA/hqdefault.jpg?sqp=-                      
  oaymwEcCNACELwBSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&rs=AOn4CLDmOFtUnDmQx5U_PKBqV819YujOBw
  ...
  • Autocompletion with vim-dadbod-completion https://github.                 
  com/kristijanhusak/vim-dadbod-completion                                    
  • Jump to foreign keys from the dadbod output (see :help                    
  ...
  After installation, run :DBUI, which should open up a drawer with all       
  databases provided.                                                         
  When you finish writing your query, just write the file (:w) and it will    
  automatically execute the query for that database.                          
  ...
  If $DBUI_URL env variable exists, it will be added as a connection. Name for
  the connection will be parsed from the url.                                 
  ...
  as an array of objects or an object:                                        
  ...
  Or if you want them to be sorted in the order you define them, this way is  
  also available:                                                             
  ...
  'postgres://postgres:mypassword@localhost:5432/my-dev-db' }                 
    \ { 'name': 'staging', 'url':                                             
  'postgres://postgres:mypassword@localhost:5432/my-staging-db' },            
  ...
  Just make sure to **NOT COMMIT** these. I suggest using project local vim   
  config (:help exrc)                                                         
                                                                              
  #### Via :DBUIAddConnection command                                         
                                                                              
  Using :DBUIAddConnection command or pressing A in dbui drawer opens up a    
  prompt to enter database url and name,                                      
  that will be saved in g:db_ui_save_location connections file. These         
  connections are available from everywhere.                                  
                                                                              
  #### Connection related notes                                               
                                                                              
  It is possible to have two connections with same name, but from different   
  source.                                                                     
  for example, you can have my-db in env variable, in g:dbs and in saved      
  connections.                                                                
  To view from which source the database is, press H in drawer.               
  If there are duplicate connection names from same source, warning will be   
  shown and first one added will be preserved.                                
                                                                              
  ## Settings                                                                 
                                                                              
  An overview of all settings and their default values can be found at :help  
  vim-dadbod-ui.                                                              
                                                                              
  ### Table helpers                                                           
                                                                              
  Table helper is a predefined query that is available for each table in the  
  list.                                                                       
  ...
  Postgres, Mysql and Sqlite has some additional helpers defined, like        
  "Indexes", "Foreign Keys", "Primary Keys".                                  
                                                                              
  Predefined query can inject current db name and table name via {table} and  
  {dbname}.                                                                   
                                                                              
  To add your own for a specific scheme, provide it through                   
  .g:db_ui_table_helpers.                                                     
                                                                              
  For example, to add a "count rows" helper for postgres, you would add this  
  as a config:                                                                
  ...
  Or if you want to override any of the defaults, provide the same name as    
  part of config:                                                             
  ...
  If this is set to 1, opening any of the table helpers will also             
  ...
  **DEPRECATED**: Use Table helpers instead.                                  
  ...
  When opening up a table, buffer will be prepopulated with some basic select,
  which defaults to:                                                          
                                                                              
    select * from table LIMIT 200;                                            
                                                                              
  To change the default value, use g:db_ui_default_query, where {table} is    
  ...
  There's a mapping to save them permanently for later to the specific        
  location.                                                                   
                                                                              
  That location is by default ~/.local/share/db_ui. To change it, addd        
  g:db_ui_save_location to your vimrc.                                        
  ...
  For queries, filetype is automatically set to sql. Also, two mappings is    
  added for the sql filetype:                                                 
  ...
  If you don't want mappings to be added, add this to vimrc:                  
    ...
    let g:db_ui_disable_mappings_javascript = 1   " Disable mappings in       
  Javascript buffers (for Mongodb)                                            
  ...
  If you want to utilize *DBExecutePre or *DBExecutePost to make your own     
  progress bar                                                                
