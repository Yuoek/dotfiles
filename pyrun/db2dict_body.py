...
class MySQLToDictExporter:
    ...
    def export_data(self, database, table, output_file):
        ...
        try:
            ...
            # 查询数据
            query = f"SELECT prefix, body FROM {table} ORDER BY prefix"
            ...
            with open(output_file, 'w', encoding='utf-8') as f:
                for item in processed_data:
                    f.write(f"{item['body']}\t{item['prefix']}\n")
            ...
            print("\n导出的数据预览:")
            print("-" * 40)
            print(f"{'prefix':<10} | {'body':<20}")
            print("-" * 40)
            for item in processed_data[:10]:  # 显示前10条记录
                print(f"{item['prefix']:<10} | {item['body']:<20}")
